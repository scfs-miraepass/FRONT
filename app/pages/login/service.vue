...
const login = async () => {
    (document.activeElement as HTMLElement)?.blur();
    if (step1.password == null? (serviceId.value ?? 0) < 5000:(password.value.length < 8 || password.value.length > 15 || (step1.password == false && password.value !== password_confirm.value))) return
